...
describe(
  "agent (deterministic fallback, LLM unconfigured)",
  () => {
    it(
      "1. produces a structured response for a valid shopping request",
      async () => {
    ...
    it(
      "2. produces a CREATE_PAYMENT proposal for a purchase request",
      async () => {
        const result = await runAgentChat({
          message: "Buy the Artisan Coffee Kit",
        });
        ...
        assert.equal(
          proposal.proposedAmountInPaise,
          179900,
        );
        ...
        assert.equal(
          String(proposal.items[0].productId),
          coffeeKitId,
        );

        /*
         * The AI requires approval, but the policy engine
         * independently enforces the merchant policy.
         */
        const policy = await evaluateAction(
          proposal,
          BASE_POLICY,
        );
    ...
    it(
      "3. does not treat the proposed price as authoritative",
      async () => {
        /*
         * Build a proposal using the real catalog.
         */
        ...
        /*
         * Simulate a malicious or buggy AI changing the price.
         */
        const tampered = {
          ...proposal,
          proposedAmountInPaise: 99900,
        };

        /*
         * The deterministic policy engine must recompute
         * the authoritative amount from MongoDB.
         */
        const policy = await evaluateAction(
          tampered,
          BASE_POLICY,
        ...
        const mismatch = policy.checks.find(
          (check) =>
            check.name === "PRICE_MISMATCH",
        );
        ...
        /*
         * Server-verified amount comes from MongoDB,
         * not from the AI proposal.
         */
        assert.equal(
          policy.verifiedAmountInPaise,
          179900,
        );
      },
    );
    ...
    it(
      "4. safely rejects malformed AI output",
      () => {
    ...
    it(
      "5. rejects AI output containing unknown product IDs",
      () => {
        const catalog = buildCatalog();

        /*
         * Unknown recommendation product.
         */
        assert.throws(
          () =>
            parseAgentOutput(
        ...
        /*
         * Unknown product inside a payment proposal.
         */
        assert.throws(
          () =>
            parseAgentOutput(
    ...
    it(
      "6. exposes no payment execution tool to the agent",
      () => {
        /*
         * The agent's tools are proposal/search/catalog tools only.
         *
         * There is intentionally no:
         * - createPayment
         * - executePayment
         * - chargeCustomer
         * - any other money-moving tool
         */
        assert.deepEqual(
          [...AGENT_TOOLS].sort(),
          [
        ...
        /*
         * proposePayment only builds a proposal.
         * It does not create an order or payment.
         *
         * Its output feeds the deterministic policy engine.
         */
      },
    );

    it(
      "searches the catalog deterministically",
      async () => {
        const results =
          await searchProducts("coffee gift");
